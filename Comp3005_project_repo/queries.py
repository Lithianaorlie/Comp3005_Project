...
def Q_1(conn, execution_time):
    ...
    query = """
    SELECT 
        ep.event_player_name,
        AVG(s.statsbomb_xg) AS average_xG
    FROM 
        event_player AS ep
    JOIN 
        events AS e ON ep.event_id = e.event_id
    JOIN 
        shot AS s ON e.event_id = s.event_id
    WHERE 
        s.statsbomb_xg > 0
    GROUP BY 
        ep.event_player_name
...
def Q_2(conn, execution_time):
    ...
    LEFT JOIN 
        shot AS s ON e.event_id = s.event_id
    WHERE 
        s.event_id IS NOT NULL AND s.statsbomb_xg > 0
    GROUP BY 
        ep.event_player_name
    HAVING 
        COUNT(s.event_id) >= 1
...
def Q_3(conn, execution_time):
    ...
    JOIN 
        shot AS s ON e.event_id = s.event_id
    WHERE 
        s.first_time = TRUE AND s.statsbomb_xg > 0
    GROUP BY 
        ep.event_player_name
    HAVING 
        COUNT(s.event_id) >= 1
...
def Q_4(conn, execution_time):
    ...
    SELECT 
        et.team_name,
        COUNT(p.event_id) AS num_passes
    FROM 
        event_team AS et
    JOIN 
        events AS e ON et.event_id = e.event_id
    JOIN 
        pass AS p ON e.event_id = p.event_id
    GROUP BY 
        et.team_name
    HAVING 
        COUNT(p.event_id) >= 1
...
def Q_5(conn, execution_time):
    ...
    SELECT 
        r.recipient_name,
        COUNT(p.event_id) AS num_passes_received
    FROM 
        recipient AS r
    JOIN 
        pass AS p ON r.recipient_id = p.recipient_id
    JOIN 
        events AS e ON p.event_id = e.event_id
    JOIN 
        event_team AS et ON e.event_id = et.event_id
    WHERE 
        et.competition_name = 'Premier League'
        AND et.season = '2003/2004'
    GROUP BY 
        r.recipient_name
    HAVING 
        COUNT(p.event_id) >= 1
...
def Q_6(conn, execution_time):
    ...
    FROM 
        team AS t
    JOIN 
        events AS e ON t.team_id = e.team_id
    JOIN 
        shot AS s ON e.event_id = s.event_id
    WHERE 
        e.competition_name = 'Premier League'
        AND e.season = '2003/2004'
    GROUP BY 
        t.team_name
    HAVING 
        COUNT(s.event_id) >= 1
...
def Q_7(conn, execution_time):
    ...
    query = """
        SELECT 
            ep.event_player_name,
            COUNT(p.event_id) AS num_through_balls
        FROM 
            event_player AS ep
        JOIN 
            pass AS p ON ep.event_player_id = p.player_id
        WHERE 
            p.through_ball = true
        GROUP BY 
            ep.event_player_name
...
def Q_8(conn, execution_time):
    ...
    query = """ 
        ...
        FROM 
            team AS t
        JOIN 
            pass AS p ON t.team_id = p.team_id
        WHERE 
            p.through_ball = true
        GROUP BY 
            t.team_name
...
def Q_9(conn, execution_time):
    ...
    query = """ 
        SELECT
            ep.event_player_name,
            COUNT(do.event_id) AS num_successful_dribbles
        FROM 
            event_player AS ep
        JOIN 
            dribble_outcome AS do ON ep.event_id = do.event_id
        WHERE 
            do.drible_outcome_name = 'Completed'
        GROUP BY 
            ep.event_player_name
        HAVING 
            COUNT(do.event_id) >= 1
...
def Q_10(conn, execution_time):
    ...
    SELECT 
    ep.event_player_name,
    COUNT(dp.event_id) AS num_dribbles_past
    FROM 
        event_player AS ep
    LEFT JOIN 
        dribble_past AS dp ON ep.event_id = dp.event_id
    GROUP BY 
        ep.event_player_name
    HAVING 
        COUNT(dp.event_id) >= 1
    ORDER BY 
        num_dribbles_past ASC;
 """
